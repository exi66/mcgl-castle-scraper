...
request({
	...
	if (body !== '') {
		...
		request({
			method: 'GET',
			url: 'https://forum.minecraft-galaxy.ru' + href + '?t=2',
			jar: localJar,
			headers: {
				'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:61.0) Gecko/20100101 Firefox/61.0'
			}
			...
			if (body !== '') {
				let json_name = href.substring(19);
				let flag = true;
				let warapper = body.match('<table class=\"topics \">(.*?)</table>')[0];
				let table = warapper.match(/<tr[\s\S]*?<\/tr>/g);
				for (let i = 1; i < table.length; i++) {
					if (flag) {
						if (table[i].includes('(элементов не найдено)')) continue;
						let _event = { 'date': '', 'player': '', 'clan': '', 'text': 'text', 'type': '', 'data': '' };
						_event.date = table[i].match('[0-9]{4}([\-/ \.])[0-9]{2}[\-/ \.][0-9]{2}[\ / \.][0-9]{2}[\:/ \.][0-9]{2}[\:/ \.][0-9]{2}')[0];
						try {
							_event.player = table[i].match(/\b(href=\"\/profilemain\/)[0-9]+/gm)[0].substring(19);
